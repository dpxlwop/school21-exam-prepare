...
int main(){
    ...
    while((string[i] = getchar()) != '\n')  //вводим строку
        i++;
    for (i = 0; string[i] == ' '; i++){}    //считаем кол-во пробелов до первого символа
    new_string[0] = string[i];      //заносим первую букву/цифру, счетчик делаем индексом занесенного значения, что бы его пропустить
    while (string[i] != '\n')   
    {
