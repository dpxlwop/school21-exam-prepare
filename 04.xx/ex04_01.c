...
int main() {
    int bin_k, n;
    if (scanf("%d", &n) != 1) return printf("n/a"); //ввод n с проверкой, по условию k - изначально 0, следует до n
    for (int k = 0; k <= n; k++) {
        bin_k = factorial(n) / (factorial(k) * factorial((n - k)));     //берем факториал и расчитываем один коэф.
        printf("%d", bin_k);        
