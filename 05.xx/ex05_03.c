...
int main() {
    ...
    while (array[counter] != -1) {
        ...
        if (scanf("%d", &array[counter]) != 1) return printf("n/a");        //ввод массива и расширение памяти для каждого элемента
    }
    int fn = 0, fk = counter - 1;   //инициализация двух переменных, которые будут учавствовать в выводе
    for (int i = 0; i < counter; i++) {
        if (i % 2 != 0) {       //идем по массиву, если индекс, на котором мы находимся НЕ четный - выводим число с конца
            printf("%d", array[fk]);
            fk--;
        } else {
